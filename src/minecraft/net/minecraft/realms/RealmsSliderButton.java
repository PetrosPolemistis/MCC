...
public class RealmsSliderButton extends RealmsButton
{
    ...
    public void renderBg(int p_renderBg_1_, int p_renderBg_2_)
    {
        if (this.getProxy().visible)
        {
            if (this.sliding)
            {
                this.value = (float)(p_renderBg_1_ - (this.getProxy().width + 4)) / (float)(this.getProxy().getXpos() - 8);
            ...
            this.blit(this.getProxy().width + (int)(this.value * (float)(this.getProxy().getXpos() - 8)), this.getProxy().height, 0, 66, 4, 20);
            this.blit(this.getProxy().width + (int)(this.value * (float)(this.getProxy().getXpos() - 8)) + 4, this.getProxy().height, 196, 66, 4, 20);
        }
    }

    public void clicked(int p_clicked_1_, int p_clicked_2_)
    {
        this.value = (float)(p_clicked_1_ - (this.getProxy().width + 4)) / (float)(this.getProxy().getXpos() - 8);
